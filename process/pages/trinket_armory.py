...
class TrinketArmoryProcessor(PageProcessor):
    def process(self):
        ...
        for raw_trinket_armory_set in self.iterate_file("trinket_gear_sets"):
            try:
                ...
                print(
                    "Cannot process trinket armory set: %s because of %s"
    ...
    def _process_trinket_armory_set(self, raw_trinket_armory_set):
        ...
        for trinket_name in raw_trinket_armory_set["trinket_names"]:
            try:
                ...
                print(
                    "Cannot process trinket with name: %s because of %s"
                    % (trinket_name, e)
                )
                continue
        bonuses = []
        for bonus_name in trinket_armory_collection["bonuses"]:
            try:
                ...
                print(
                    "Cannot process bonus with name: %s because of %s" % (bonus_name, e)
                )
                continue
        return {
    ...
    def _process_bonus(self, bonus_name):
        armory_stat = self.lookup_files(
            ["armory_stat_sets_1", "armory_stat_sets_2"], "name", bonus_name
        )["stats"][0]
        return {
            "name": self.translate(
    ...
    def _process_gear_with_level(self, gear_name_with_level):
        gear_with_level = self.lookup_files(
            ["gears_1", "gears_2", "trinket_gears"], "name", gear_name_with_level
        )
        ...
        gear_stat_set = self.lookup_files(
            ["gear_stat_sets_1", "gear_stat_sets_2", "trinket_gear_stat_sets"],
            "name",
            gear_name_with_level,
        )
    ...
    def _process_gear_stat(self, stat_name, stat_progression_name):
        ...
        if not stat_progression:
            raise InsufficientDataException(
                "Cannot find stat progression %s" % stat_progression_name
            )
        return {
            "name": self.translate(
